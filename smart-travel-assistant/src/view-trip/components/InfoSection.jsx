...
function InfoSection({ trip }) {
  ...
  useEffect(() => {
    const fetchPlacePhoto = async () => {
      if (!trip || !trip.tripData || !trip.tripData.tripPlan.destination)
        return;
    ...
    <div>
      ...
      <div className="my-5 flex flex-col gap-4">
        <div className="flex justify-between flex-wrap items-center">
          ...
          <h2 className="font-bold text-2xl">
            {" "}
            <FontAwesomeIcon icon={faCloud} className="text-violet-600 mr-2" />
            {trip?.tripData?.tripPlan?.weatherForecast?.[0].Min} -{" "}
            {trip?.tripData?.tripPlan?.weatherForecast?.[0].Max}
          </h2>
        </div>
        ...
        <div className="flex justify-between flex-wrap items-center gap-2">
          ...
          <h2 className="text-gray-500 text  text-right">
            {trip?.tripData?.tripPlan?.weatherForecast?.[0].summary}
          </h2>
        </div>
      </div>
    </div>
